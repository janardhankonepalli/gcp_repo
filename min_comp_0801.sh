...
bq_partitioned_table="shc-pricing-prod:min_comp_tables.min_comp_all"'_'"${hour_partition}"
bq query --destination_table=$bq_partitioned_table "
select aaa.div_no div_no , aaa.itm_no itm_no , bbb.min_comp min_comp, aaa.min_comp_name  min_comp_NM, bbb.max_comp max_comp, bbb.avg_comp avg_comp from 
(select div_no , itm_no , price , min(comp_name) min_comp_name from 
(
...
FROM TABLE_QUERY([shc-pricing-prod:all_comp_tables], 
      ...
      AND table_id= (Select MAX(table_id) 
                              ...
                              where table_id contains \"all_comp_all\")')
where (
...
group by 1,2,3) aaa
join
(select div_no , itm_no , min(price) min_comp, max(price) max_comp, avg(price) avg_comp from 
(
SELECT *, rank() over (PARTITION BY concat order by last_crawled_date desc) rank_
FROM TABLE_QUERY([shc-pricing-prod:all_comp_tables], 
      'table_id CONTAINS \"all_comp_all\" 
