...
bq_partitioned_table="shc-pricing-prod:rule_tables.Div8_10pct_pricing"'_'"${hour_partition}"
...
(case when a.blocked is null and (a.ad_plan <> 'Y' or a.ad_plan <> 'Y' is null) and a.cost_with_subsidy is not null and a.reg is not null and a.reg <> 0
 then (case when b.min_comp is not null and b.min_comp >= a.cost_with_subsidy /0.9
	  then b.min_comp 
	  else a.cost_with_subsidy /0.9
	  end )
end) Recom_prc,
(case when a.blocked is null and (a.ad_plan <> 'Y' or a.ad_plan <> 'Y' is null) and a.cost_with_subsidy is not null and a.reg is not null and a.reg <> 0
 then (case when b.min_comp is not null and b.min_comp >= a.cost_with_subsidy /0.9
	  then 'Home match at 10%'
	  else 'Home unmatched at 10%'
	  end )
end) Rule_name,
'5-Item' Rule_Level
from 
...
join [shc-pricing-dev:static_lists.Home_10pct_Pricing_20160805] d
