...
bq_partitioned_table="shc-pricing-prod:static_tables.static__table"'_'"${day_partition}"
...
left join each
(  
SELECT integer(ltrim(rtrim(div_no))) div_no, integer(ltrim(rtrim(item_no))) itm_no, sum(integer(ltrim(rtrim(visits)))) visits_30, sum(integer(ltrim(rtrim(orders)))) orders_30, 
sum(integer(ltrim(rtrim(orders))))/sum(integer(ltrim(rtrim(visits)))) conversion_30  FROM [shc-pricing-prod:bq_pricing_it.smith_omniture_data] where location_number  ='9300'  
...
"
bq_partitioned_table="shc-pricing-prod:static_tables.static_table_MM"'_'"${day_partition}"
bq query --destination_table=$bq_partitioned_table " 
select *,  
